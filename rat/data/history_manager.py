...
class HistoryManager:
    ...
    def get_global_panel(self, start, end, period=300, features=('close',)):
        ...
        multi_index = pd.MultiIndex.from_product([coins, features],
                                                 names=['coin', 'feature'])
...
def panel_fillna(panel, type="bfill"):
    ...
    """
    for coin in panel.columns.get_level_values("coin").unique():
        if type == "both":
            panel[coin] = panel[coin].fillna(method="bfill").fillna(method="ffill")
        else:
            panel[coin] = panel[coin].fillna(method=type)
    return panel
