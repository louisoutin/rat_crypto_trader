...
class Poloniex:
    def __init__(self, APIKey='', Secret=''):
        ...
        self.marketChart = lambda pair, period=day, start=time.time()-(week*1), end=time.time(): \
            self.api('returnChartData', {'currencyPair': pair, 'period': period, 'start': start, 'end': end})
        self.marketTradeHist = lambda pair: \
            self.api('returnTradeHistory', {'currencyPair': pair})  # NEEDS TO BE FIXED ON Poloniex
